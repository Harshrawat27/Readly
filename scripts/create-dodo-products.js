...
async function createProducts() {
  ...
  for (const plan of PLANS) {
    try {
      ...
      console.log(`   Price: $${product.price_amount / 100} ${product.price_currency}`);
      console.log('');
  ...
  try {
    ...
    if (products.items && products.items.length > 0) {
      products.items.forEach(product => {
        console.log(`📦 ${product.name} - $${product.price_amount / 100} (${product.id})`);
      });
    } else {
...
async function main() {
  if (!process.env.DODO_API_KEY_TEST && !process.env.DODO_API_KEY_LIVE) {
    console.error('❌ Missing Dodo Payments API key. Set DODO_API_KEY_TEST or DODO_API_KEY_LIVE in your .env file');
    process.exit(1);
  }

  console.log('🔑 Using Dodo Payments in:', process.env.NODE_ENV === 'production' ? 'LIVE mode' : 'TEST mode');
  console.log('');
  ...
  rl.question('Do you want to create the subscription products? (y/N): ', async (answer) => {

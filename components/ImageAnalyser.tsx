...
interface ImageAnalyserProps {
  ...
  onHighlight: (color: string, area: { x: number; y: number; width: number; height: number }) => void;
}
...
const highlightColors = [
  { name: 'Yellow', color: '#FDE047', bg: 'bg-yellow-400', hover: 'hover:bg-yellow-500' },
  { name: 'Green', color: '#4ADE80', bg: 'bg-green-400', hover: 'hover:bg-green-500' },
  { name: 'Blue', color: '#60A5FA', bg: 'bg-blue-400', hover: 'hover:bg-blue-500' },
  { name: 'Pink', color: '#F472B6', bg: 'bg-pink-400', hover: 'hover:bg-pink-500' },
  { name: 'Purple', color: '#A78BFA', bg: 'bg-purple-400', hover: 'hover:bg-purple-500' },
];
...
export default function ImageAnalyser({
  ...
  const captureSelectedArea = useCallback((area: SelectionArea) => {
    ...
    const canvasStartX = (Math.min(area.startX, area.endX) - (canvasRect.left - containerRect.left)) * scaleX;
    const canvasStartY = (Math.min(area.startY, area.endY) - (canvasRect.top - containerRect.top)) * scaleY;
    ...
    tempCtx.drawImage(
      canvas,
      canvasStartX, canvasStartY, canvasWidth, canvasHeight,
    ...
    if (width > 10 && height > 10) {
      ...
      if (imageData) {
        ...
        setAnalysisDialog({
          ...
          imageData,
          area: {
            x: (Math.min(finalArea.startX, finalArea.endX) / rect.width) * 100,
            y: (Math.min(finalArea.startY, finalArea.endY) / rect.height) * 100,
  ...
  const handleHighlightArea = useCallback((color: string) => {
  ...
  return (
    <>
          ...
          <>
            ...
            <div
              ...
              style={{
                ...
                width: '100%',
                height: `calc(100% - ${Math.max(selectionArea.startY, selectionArea.endY)}px)`,
              }}
            />
            {/* Left overlay */}
            <div
            ...
            <div
              ...
              style={{
                ...
                top: Math.min(selectionArea.startY, selectionArea.endY),
                width: `calc(100% - ${Math.max(selectionArea.startX, selectionArea.endX)}px)`,
                height: Math.abs(selectionArea.endY - selectionArea.startY),
              }}
            />
            {/* Selection border */}
      ...
      {analysisDialog.visible && (
        <>
          {/* Backdrop */}
          <div className="fixed inset-0 z-40" onClick={handleAnalysisDialogClose} />
          
          {/* Dialog */}
          <div
            className="fixed z-50 bg-white rounded-lg shadow-lg border border-gray-200 p-3 analysis-dialog"
            style={{
              left: Math.max(10, Math.min(analysisDialog.x - 120, window.innerWidth - 250)),

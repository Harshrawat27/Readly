...
export default function ShapeTool({
  ...
  const isShapeToolActive = useCallback(() => {
    return isActive && ['rectangle', 'ellipse', 'line', 'arrow', 'move'].includes(activeTool);
  }, [isActive, activeTool]);

  const isDrawingTool = useCallback(() => {
    return isActive && ['rectangle', 'ellipse', 'line', 'arrow'].includes(activeTool);
  }, [isActive, activeTool]);

  // Create shape via API
  const createShape = useCallback(async (shapeData: Partial<Shape>) => {
    try {
  ...
  const updateShape = useCallback(async (shapeId: string, updates: Partial<Shape>) => {
    try {
  ...
  const getMousePosition = useCallback((e: MouseEvent | React.MouseEvent) => {
    ...
    const targetElement = containerRef.current || (e.target as HTMLElement)?.closest('[data-page-number]') || e.target as HTMLElement;
  ...
  const isPointInShape = useCallback((x: number, y: number, shape: Shape) => {
    switch (shape.type) {
      ...
      case 'arrow':
        ...
        return lineDistance <= (shape.strokeWidth || 2) + 5;
      default:
        return false;
    }
  }, []);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if ((handleType || endpointType) && selectedShapeId) {
      ...
      if (clickedShape) {
        if (endpointType && (clickedShape.type === 'line' || clickedShape.type === 'arrow')) {
          ...
          setDrawingState({
            ...
            resizing: {
              shape: clickedShape,
              handle: handleType as 'nw' | 'ne' | 'sw' | 'se' | 'n' | 's' | 'e' | 'w',
              startX: x,
              startY: y,
              startShapeX: clickedShape.x,
              startShapeY: clickedShape.y,
    ...
    if (clickedShape) {
      ...
      console.log('Starting shape creation on mousedown:', shapeType, 'at', x, y);
      setDrawingState({
        isDrawing: true,
        startX: x,
        startY: y,
  ...
  }, [isShapeToolActive, getMousePosition, activeTool, shapes, isPointInShape, selectedShapeId]);
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (drawingState.isDrawing) {
      ...
      console.log('Drawing preview update:', x, y);
      ...
      const { shape, handle, startX, startY, startShapeX, startShapeY, startShapeWidth, startShapeHeight } = drawingState.resizing;
      const dx = x - startX;
      const dy = y - startY;
      ...
      // Ensure minimum size
      newWidth = Math.max(newWidth, MIN_SHAPE_SIZE);
      newHeight = Math.max(newHeight, MIN_SHAPE_SIZE);
      ...
      const { shape, endpoint, startX, startY, startShapeX, startShapeY, startShapeWidth, startShapeHeight } = drawingState.editingLineEndpoint;
      const dx = x - startX;
      const dy = y - startY;
      ...
      // Also update via API with debouncing
      debouncedUpdateShape(drawingState.draggedShape.id, { x: newX, y: newY });
    }
  }, [isShapeToolActive, getMousePosition, drawingState, debouncedUpdateShape, onShapeUpdate]);
  ...
  const handleMouseUp = useCallback(async (e: MouseEvent) => {
    ...
    if (drawingState.isDrawing) {
      ...
      console.log('Mouse up - completing shape creation:', shapeType, 'from', drawingState.startX, drawingState.startY, 'to', x, y);
      ...
      let shapeX = Math.min(drawingState.startX, x);
      let shapeY = Math.min(drawingState.startY, y);
      let shapeWidth = Math.abs(x - drawingState.startX);
      let shapeHeight = Math.abs(y - drawingState.startY);

      // Check if this was a drag operation (moved more than 5 pixels)
      const wasDragged = shapeWidth > 5 || shapeHeight > 5;

      if (!wasDragged) {
        // Single click - create default size shape centered at click point
        shapeX = drawingState.startX - DEFAULT_SHAPE_SIZE / 2;
        shapeY = drawingState.startY - DEFAULT_SHAPE_SIZE / 2;
        shapeWidth = DEFAULT_SHAPE_SIZE;
        
        if (shapeType === 'line' || shapeType === 'arrow') {
          // For lines/arrows, create a horizontal line of default size
          shapeX = drawingState.startX;
          shapeY = drawingState.startY;
          shapeWidth = DEFAULT_SHAPE_SIZE;
          shapeHeight = 0;
        } else {
          shapeHeight = DEFAULT_SHAPE_SIZE;
        }
      } else {
        // Dragged - ensure minimum size for non-line shapes
        if (shapeType !== 'line' && shapeType !== 'arrow') {
          shapeWidth = Math.max(shapeWidth, MIN_SHAPE_SIZE);
          shapeHeight = Math.max(shapeHeight, MIN_SHAPE_SIZE);
        }
      }
  ...
  const renderDrawingPreview = () => {
    ...
    switch (shapeType) {
      ...
      case 'arrow':
        ...
        const arrowHeadSize = Math.min(20, arrowLength * 0.2);

        const arrowHeadX1 = drawingState.currentX - arrowHeadSize * Math.cos(angle - Math.PI / 6);
        const arrowHeadY1 = drawingState.currentY - arrowHeadSize * Math.sin(angle - Math.PI / 6);
        const arrowHeadX2 = drawingState.currentX - arrowHeadSize * Math.cos(angle + Math.PI / 6);
        const arrowHeadY2 = drawingState.currentY - arrowHeadSize * Math.sin(angle + Math.PI / 6);
        ...
        return (
          <g opacity={0.7}>
            ...
            />
            <polygon
              points={`${drawingState.currentX},${drawingState.currentY} ${arrowHeadX1},${arrowHeadY1} ${arrowHeadX2},${arrowHeadY2}`}
              fill={shapeColor}
            />
          </g>
        );
      ...
      .map(shape => {
        ...
        switch (shape.type) {
          ...
          case 'arrow':
            ...
            shapeElement = (
              <g {...commonProps}>
                <line x1={shape.x} y1={shape.y} x2={endX} y2={endY} />
                <polygon
        ...
        return (
          <g key={shape.id} style={{ pointerEvents: 'auto' }}>
            {shapeElement}
            {isSelected && (
  ...
  return (
    ...
    >
      <svg
        ...
        style={{ 
          zIndex: 21,
          pointerEvents: isShapeToolActive() ? 'auto' : 'none' // Auto only when shape tool active
        }}
      >
        {renderShapes()}

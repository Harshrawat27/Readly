...
export async function findRelevantChunks(
  ...
  try {
    ...
    console.log(`🔍 Finding relevant chunks for query: "${query.substring(0, 100)}${query.length > 100 ? '...' : ''}"`);
    ...
    console.log(`📊 Total chunks with embeddings for PDF ${pdfId}: ${totalChunksWithEmbeddings}`);
    
    if (totalChunksWithEmbeddings === 0) {
      console.log('⚠️ No chunks have embeddings! Falling back to regular chunks');
    ...
    const chunks = await prisma.$queryRawUnsafe<Array<{
      ...
      LIMIT $4
    `, queryVector, pdfId, similarityThreshold, limit);
    ...
    if (chunks.length > 0) {
      console.log('📊 Top similarity scores:', chunks.slice(0, 3).map(c => `${c.similarity?.toFixed(3)}`).join(', '));
    } else {
      ...
      try {
        const allScores = await prisma.$queryRawUnsafe<Array<{
          similarity: number;
          pageNumber: number;
        }>>(`
          WITH query_vector AS (SELECT $1::vector as vec)
        ...
        console.log('📊 All similarity scores (top 5):', 
          allScores.map(s => `Page ${s.pageNumber}: ${s.similarity?.toFixed(3)}`).join(', '));
      } catch (debugError) {
    ...
    if (chunks.length === 0) {
      console.log('⚠️ No chunks met similarity threshold, falling back to first chunks');
      const fallbackChunks = await prisma.pDFChunk.findMany({
        where: { pdfId },
        orderBy: { chunkIndex: 'asc' },
        take: limit,
...
export async function searchAcrossPDFs(
  ...
  try {
    ...
    console.log(`🔍 Searching across user's PDFs for: "${query.substring(0, 100)}${query.length > 100 ? '...' : ''}"`);
    const queryEmbedding = await generateQueryEmbedding(query);
